...
class Qwen3(BaseModelArch):
    """
    Implementation of the Qwen3 model architecture.

    This class represents the Qwen3-235B-A22B model with Mixture-of-Experts (MoE)
    and Grouped Query Attention (GQA), providing methods to access its
    parameters and KV cache configuration.
    """
    ...
    def __init__(
        ...
        """
        Initialize a Qwen3 model architecture.

        Args:
            model_name (str): The model identifier.
            num_layers (int): Number of transformer layers.
            num_query_heads (int): Number of query heads (64 for Qwen3-235B-A22B).
            num_kv_heads (int): Number of key-value heads (4 for Qwen3-235B-A22B).
    ...
    def get_kv_size_per_token(self, token_count: int = 1) -> int:
        """
        Get the key-value cache size for the Qwen3 model (per token).

        The KV cache is based on the Grouped Query Attention mechanism:
        - Uses 4 key-value heads (not affected by MoE)
        - Each head has configurable dimension
    ...
    def get_io_size(self, page_size: int = 1) -> int:
        """
        Calculates the IO size for one token per GPU for the Qwen3 model.
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        Convert the Qwen3 model configuration to a dictionary.
    ...
    def __str__(self) -> str:
        """
        Get a string representation of the Qwen3 model.
