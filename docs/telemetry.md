...
2. **Shared Memory Buffer**: Statically-linked built in implemenation of telemetry exporter. Uses shared memory cyclic buffer for efficient event storage and export.
...
The **Shared Memory Buffer** plug-in, contains the data per transacation event, without summarizing between events.
